...
連線設定只作為本次使用的入口頁（`/login`）。使用者儲存設定後，前端自動跳轉至 Dashboard（`/dashboard`）；如需調整連線，可由側邊欄「連線設定」返回入口頁。Token 存入 `sessionStorage`（同一瀏覽器進程內有效，不跨重啟）；Project ID 存入 `localStorage`（持久跨頁）。頁面刷新不需重新輸入。
...
- 共享靜態資源：`static/style.css`（全域樣式）、`static/app.js`（業務邏輯）、`static/ui.js`（UI 共用函式）
...
| 版本 | 日期 | 說明 |
|------|------|------|
| v1.2.7 | 2026-04-19 | 狀態保持與體驗修復：加入 `sessionStorage` 恢復 Dashboard 狀態避免跳轉清空、補齊 Issue Grid Table 的排序/過濾關閉/動態 Click Popup 功能、修復背景載入無反應字樣殘留 |
| v1.2.1 | 2026-04-19 | 架構重構：將 app.py 與 index.html 模組化拆分為 modules/ 與 static/ 資料夾結構 |
