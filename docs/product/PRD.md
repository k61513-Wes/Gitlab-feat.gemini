...
- **多來源支援（Redmine）**：新增 Redmine Issue 抓取支援，透過 `RedmineClient` 與專屬 API 端點實現紅米 Issues 的自動化處理（目前為後端基礎整合階段）。
...
| `routes/redmine.py` | `/api/redmine/*` | Redmine 連線檢查、Issue 抓取與列表解析 |
...
| `/api/redmine/health` | GET | Redmine 連線健康檢查 |
| `/api/redmine/issue` | POST | 取得 Redmine 單筆 Issue |
| `/api/redmine/issues` | POST | 取得 Redmine Issue 列表 |
