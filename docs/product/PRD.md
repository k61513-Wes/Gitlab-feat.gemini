...
**操作邏輯**：
- 「檢查 Gemini CLI」按鈕呼叫 `GET /api/health`，顯示 CLI 可用性與模型選項
- 「儲存」後，Token 寫入 `sessionStorage`，Project ID（若勾選記住）寫入 `localStorage`
...
- 右側欄可查看格式轉換後的 JSON 輸出（Export 結果），並支援複製與下載 .txt
...
| LLM 整合 | Gemini CLI subprocess（`gemini_cli.py` 封裝） |
| Excel 生成 | openpyxl 3.1+（`excel_utils.py`） |
| 設定管理 | `modules/config.py`（環境變數 + 常數 + 日誌） |

**後端模組一覽**：

| 模組/路由 | 路徑 | 責任 |
|-----------|------|------|
| `config.py` | `modules/config.py` | APP_VERSION、環境變數、日誌、模型鏈設定、System Prompt 常數 |
| `gemini_cli.py` | `modules/gemini_cli.py` | CLI subprocess 封裝、Flash 禁用（`is_disallowed_model`）、模型探針 |
...
| `routes/health.py` | `/api/health`, `/api/probe_models` | CLI 健康檢查、模型探針 |
...
- 需具備 Gemini CLI（`npm install -g @google/gemini-cli`）
