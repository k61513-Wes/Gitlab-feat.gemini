...
## v1.5.1 — UI 煥然一新：Apple Design System 導入
**更新日期：2026-04-30**

### 核心功能更新
- **視覺語言完全重構**：導入 Apple 官方 Web 設計規範，全面轉向「極簡畫布、細緻間距、內容優先」風格。
- **Action Blue 統一提示**：將全站主視覺與互動焦點統一為 Action Blue (#0066cc)，精確傳達「可點擊性」。
- **圓角體系規範化**：
  - **膠囊按鈕 (Pill)**：主要動作按鈕升級為弧度 9999px 的膠囊造型。
  - **工具卡片 (LG)**：儀表板與工作區卡片改用 18px 舒適圓角。
- **Typography 質感提升**：
  - **Apple Tight 標題**：標題字級導入負值字母間距 (-0.02em) 營造緊湊專業感。
  - **內文閱讀性**：基礎內文字級提升至 14px，行高放寬至 1.47。
- **微互動回饋**：所有點擊動作由傳統位移改為 scale(0.95) 的全域縮合反應。
- **毛玻璃效果**：Header 導入 backdrop-filter blur (20px) 效果。

### 業務邏輯保留
- **標籤色一致性**：在極簡的黑白灰底色中，精確保留了 Priority 與 Team 標籤的原始辨識色。
